...
export default function ChatSidebar({ chats, activeId, onSelect, onNew }: Props) {
  return (
    <aside className="w-[260px] h-full flex flex-col bg-surface border-r border-border overflow-hidden shrink-0">
      ...
      <div className="px-3 mb-2">
        <button onClick={onNew} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-[10px] bg-primary text-white text-[13px] font-medium hover:bg-primary-light transition-colors"><IPlus/> New Chat</button>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-2">
        <div className="mb-4">
          ...
          <div className="mt-0.5 space-y-0.5">
            {chats.map(c => (
              <button key={c.id} onClick={() => onSelect(c.id)} className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] transition-all duration-150 text-left group ${activeId === c.id ? "bg-primary/5 text-primary font-medium" : "text-text-secondary hover:bg-bg hover:text-text-primary"}`}>
