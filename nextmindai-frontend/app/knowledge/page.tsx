...
export default function KnowledgePage() {
  ...
  useClickOutside(collectionMenuRef, () => setShowCollectionMenu(false));
        ...
        <div className="max-w-[960px] mx-auto px-8 py-10">
          ...
          <div className="flex items-center gap-3 mb-3 animate-fade-in" style={{ animationDelay: "50ms" }}>
            ...
            <div className="relative" ref={collectionMenuRef}>
              <button
                onClick={() => { setShowCollectionMenu(!showCollectionMenu); setShowSortMenu(false); }}
              ...
              {showCollectionMenu && (
                <div className="absolute top-11 right-0 w-48 bg-surface border border-border rounded-[10px] shadow-lg py-1.5 z-50 animate-slide-up max-h-64 overflow-y-auto">
                  {collectionOptions.map((opt) => (
                    <button
                      key={opt}
                      onClick={() => { setActiveCollection(opt); setShowCollectionMenu(false); }}
            ...
            <div className="relative" ref={sortMenuRef}>
              <button
                onClick={() => { setShowSortMenu(!showSortMenu); setShowCollectionMenu(false); }}
                className="h-10 px-3.5 rounded-[10px] bg-surface border border-border text-[13px] text-text-secondary hover:text-text-primary hover:border-primary/20 transition-all flex items-center gap-2"
